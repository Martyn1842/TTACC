...
specification.new = function(objTyp, ...) --Creat and assign new component
    ...
    local newObj = setmetatable({}, component[objTyp])
    ...
    if objTyp == "func" then --(function: writeN, readN, passThrough)
        ...
        newObj.passThrough = (select(2, ...)) or newObj.passThrough
    end
    ...
    if newObj.writeN > 0 then --Assign write addresses
        if specification.write + newObj.writeN > 21 then
    ...
    if newObj.readN > 0 then --Assign read addresses
        if specification.read + newObj.readN > 32 then
            abort("insufficient read addresses abailable")
        end
